...
test("modifyCodeByCommand - addElementAsSiblingInArray (empty array)", () => {
  ...
  const cmd1 = modifyCodeByCommand(makeCmd(), targ, txt, undefined)!;
...
test("modifyCodeByCommand - addElementAsSiblingInArray", () => {
  ...
  const cmd1 = modifyCodeByCommand(makeCmd(), key1, exampleData, undefined)!;
  ...
  const cmd2 = modifyCodeByCommand(makeCmd(), key2, exampleData, undefined)!;
  ...
  const cmd3 = modifyCodeByCommand(makeCmd(), key3, exampleData, undefined)!;
...
test("modifyCodeByCommand - removeObjectKey", () => {
  ...
  ["a", "d", "I", "f", "g"].forEach((text) => {
    ...
    const cmd = modifyCodeByCommand(makeCmd(), key, exampleData, undefined)!;
    expect(insertSwap(exampleData, cmd)).toMatchSnapshot();
  });
  ...
  [
    ...
    const updated = insertSwap(
      exampleData,
      modifyCodeByCommand(makeCmd(), key1, exampleData, undefined)!
    );
    const key2 = findNodeByText(updated, `"${secondKey}"`)!;
    const cmd = modifyCodeByCommand(makeCmd(), key2, updated, undefined)!;
...
test("modifyCodeByCommand - removeElementFromArray", () => {
  ...
  [
    ...
    const key1 = findNodeByText(exampleData, `${firstKey}`)!;
    const cmd0 = modifyCodeByCommand(makeCmd(), key1, exampleData, undefined);
    const update1 = insertSwap(exampleData, cmd0!);
    const key2 = findNodeByText(update1, `${secondKey}`)!;
    const cmd = modifyCodeByCommand(makeCmd(), key2, update1, undefined)!;
    const update2 = insertSwap(update1, cmd);

    const key3 = findNodeByText(update2, `${thirdKey}`)!;
    const cmd2 = modifyCodeByCommand(makeCmd(), key3, update2, undefined)!;
